...
describe('JsonApi.Client', () => {
  ...
  describe('fetchResourceCollection', () => {
    ...
    test('calls request on the httpAdapter', () => {
      client.fetchResourceCollection(url)
      ...
      describe('when media type is application/vnd.api+json', () => {
        describe('when body is document with resource collection', () => {
          ...
          test('resolves result success with document', async () => {
            await expect(client.fetchResourceCollection(url)).resolves.toEqual({
              isSuccess: true,
        ...
        describe('when body is not a resource collection document', () => {
          beforeEach(() => {
            response = {
              headers: {
                'Content-Type': 'application/vnd.api+json'
