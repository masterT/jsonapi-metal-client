...
describe('JsonApi.Client', () => {
  ...
  describe('fetchRelationship', () => {
    const url = 'http://example.com/articles/1/relationships/to-one-or-to-many-relationship'
      ...
      describe('when media type is application/vnd.api+json', () => {
        describe('when body is document with relationship to-one', () => {
          ...
          test('resolves result success with document', async () => {
            await expect(client.fetchRelationship(url)).resolves.toEqual({
              isSuccess: true,
        ...
        describe('when body is not an relationship document', () => {
          beforeEach(() => {
            response = {
              headers: {
                'Content-Type': 'application/vnd.api+json'
