...
export class Client {
   ...
   * @returns
   */
  private async requestFetchData<D>(
    url: string,
  ...
  ): Promise<[HttpAdapter.AdapterRequest, HttpAdapter.AdapterResponse]> {
    const request = {
      ...options,
      headers: {
  ...
  private validateResponseMediaType(response: HttpAdapter.AdapterResponse) {
    if (response.body && response.body.trim().length > 0) {
      ...
      if (
        !contentType ||
        contentType.indexOf('application/vnd.api+json') !== 0
      ) {
        return false;
      }
    }
